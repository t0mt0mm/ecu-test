...
class DummyBackend(BackendBase):
    ...
    def _update_runtime(self, runtime: ChannelRuntime, dt: float) -> None:
        ...
        if channel_type in {"HighSide", "LowSide"}:
            ...
            target = float(sim_params.get("target", runtime.setpoint))
            runtime.status_cache["feedback"] = max(0.0, min(100.0, target + random.uniform(-noise, noise)))
        elif channel_type == "DO":
            noise = float(sim_params.get("noise", 0.05))
            pattern_period = float(sim_params.get("pattern_period", 1.0))
            pattern_duty = float(sim_params.get("pattern_duty", 0.5))
            phase = (self._time % max(pattern_period, 0.1)) / max(pattern_period, 0.1)
            pattern_on = phase < pattern_duty
            state = runtime.enabled or pattern_on
            runtime.status_cache["feedback"] = 1.0 if state else 0.0
            runtime.status_cache["state"] = runtime.status_cache["feedback"]
            runtime.status_cache["feedback"] += random.uniform(-noise, noise)
        elif channel_type == "DI":
            pattern_period = float(sim_params.get("pattern_period", 2.0))
            pattern_duty = float(sim_params.get("pattern_duty", 0.4))
            phase = (self._time % max(pattern_period, 0.1)) / max(pattern_period, 0.1)
            pattern_on = phase < pattern_duty
            runtime.status_cache["feedback"] = 1.0 if pattern_on else 0.0
        elif channel_type == "AI_V":
            noise = float(sim_params.get("noise", 0.1))
            slope = float(sim_params.get("slope", 0.5))
            runtime.current_value += slope * dt
            runtime.status_cache["value"] = runtime.current_value + random.uniform(-noise, noise)
        elif channel_type == "AI_I":
            noise = float(sim_params.get("noise", 0.1))
            slope = float(sim_params.get("slope", 0.5))
            runtime.current_value += slope * dt
            runtime.status_cache["value"] = runtime.current_value + random.uniform(-noise, noise)
    ...
    def _apply_runtime_to_signals(self, runtime: ChannelRuntime) -> None:
        ...
        if not profile.status.message:
            return
        for semantic, signal in profile.write.fields.items():
            if semantic == "select":
                self._set_override(signal, runtime.enabled)
            elif semantic == "pwm":
                self._set_override(signal, runtime.pwm if runtime.enabled else 0.0)
            elif semantic == "setpoint":
                self._set_override(signal, runtime.setpoint)
            elif semantic == "direction":
                self._set_override(signal, runtime.direction)
